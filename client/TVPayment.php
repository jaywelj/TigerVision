...
<html lang="en">
					...
					<table border="0">
						<div class="form-group">
							<tr> 
                                ...
                                type="text" name="txtbxPayerName" id="txtbxPayerName" class="form-control">
							</tr>
						</div>
						<div class="form-group">
							<tr> 
								<label>Reservation Number</label>
								<input type="number" required step="1" min="1" name="txtbxReservationNo" id="txtbxReservationNo" class="form-control">
							</tr>
						</div>
						<div class="form-group">
							<tr> 
								<label>Receipt/Payment Slip</label>
								<input type="file" accept="image/x-png,image/gif,image/jpeg" required name="txtbxReciept" id="txtbxReciept" class="form-control">
							</tr>
						</div>
						<div class="form-group">
							<tr>
	...
	if(isset($_POST['confirmPackage']))
	{
		...
		$varcharPayerName=$_POST['txtbxPayerName'];
		$varcharPaymentImage = addslashes(file_get_contents($_FILES["txtbxReciept"]["tmp_name"]));
		/*	$check = getimagesize($_FILES["txtbxReciept"]["tmp_name"]);
			$uploadOk = 0;
		    if($check !== false)
		    {
				$uploadOk = 1;
		...
		$queryInsertingInitialPayment = "INSERT INTO `tbl_payments` (`payment_ID`, `payment_name`, `payment_proof`, `payment_amount`, `payment_date`, `payment_method`, `payment_type`, `payment_reservationID`, `payment_processed`, `payment_emailStatus`,`payment_purgeFlag`) VALUES (NULL, '$varcharPayerName', '$varcharPaymentImage', NULL, NULL, NULL, NULL, '$varcharReservationID', '1', 'Not Emailed', 1)";
		if(mysqli_query($connect, $queryInsertingInitialPayment)){
			echo "<script type='text/javascript'>alert('Successfully Paid');</script>";
		}
		else{
...
<script type="text/javascript">
	$(document).ready(function(){
		$('.btnAddPayment').click(function(){
			...
			//var validationImage = "<?php $uploadOk; ?>";
			if(payerName=="")
			{
			...
			else if(recieptImage=="")/*||validationImage == 0*/
			{
